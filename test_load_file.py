...
def test_load_file_multiple_column_mappings(mock_env):
    with patch("common.read_query_from_postgres") as mock_postgres, \
         *common_patches:
        ...
        # simulate multiple db_col_name mappings
        mock_read_parquet.collect.return_value = [
            {"file_col_name": "colA", "db_col_name": "newA,newB"}
        ]
...
def test_load_file_measure_column_filtering(mock_env):
    with patch("common.read_query_from_postgres") as mock_postgres, \
         *common_patches:

        mock_spark, mock_dbutils, mock_read_parquet, mock_df = mock_env
        mock_df.columns = ["measure1", "measure2"]
        mock_dbutils.fs.ls.return_value = []

        # simulate measure columns
        mock_postgres.return_value.select.return_value.distinct.return_value.collect.return_value = [
            {"measr_phys_name": "measure1"},
